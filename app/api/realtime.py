...
class RealtimeChannel:
    ...
    def __init__(self, channel_name: str, db_client: Client):
    ...
    def subscribe(self, event_type: str, table: str, callback):
        ...
        self.channel = self.db_client.channel(self.channel_name)
    ...
    def unsubscribe(self):
        ...
        if self.channel:
            self.db_client.remove_channel(self.channel)
...
async def subscribe_to_salon(salon_id: int, db: Client = Depends(get_db_client)):
    """
    ...
    if channel_name not in salon_channels:
        channel = RealtimeChannel(channel_name, db)
...
async def subscribe_to_bookings(salon_id: int, db: Client = Depends(get_db_client)):
    """
