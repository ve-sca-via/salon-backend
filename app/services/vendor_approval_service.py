...
class VendorApprovalService:
    ...
    async def _get_vendor_request(self, request_id: str) -> Dict[str, Any]:
        """Get and validate vendor request"""
        response = db.table("vendor_join_requests").select("*").eq("id", request_id).single().execute()
    ...
    async def _get_approval_config(self) -> Dict[str, Any]:
        ...
        # Get RM score
        rm_score_response = db.table("system_config").select("config_value").eq(
        ...
        fee_response = db.table("system_config").select("config_value").eq(
    ...
    async def _update_request_status(self, request_id: str, admin_notes: Optional[str], admin_id: Optional[str] = None) -> None:
        ...
        db.table("vendor_join_requests").update(update_data).eq("id", request_id).execute()
    
    async def _geocode_salon_address(self, request_data: Dict[str, Any]) -> Dict[str, float]:
        """
    ...
    async def _create_salon(
        ...
        salon_data = {
            ...
            "registration_fee_amount": config["registration_fee"],
            "registration_fee_paid": False,
            "is_active": False,
            "approved_at": "now()"
        }
        
        response = db.table("salons").insert(salon_data).execute()
    ...
    async def _create_salon_services(
        ...
        try:
            ...
            if services_to_insert:
                ...
                response = db.table("services").insert(services_to_insert).execute()
    ...
    async def _update_rm_score(
        ...
        # Get current RM values
        rm_response = db.table("rm_profiles").select(
            "total_score, total_approved_salons"
        ).eq("id", rm_id).single().execute()
        
        current_score = rm_response.data.get("total_score", 0) if rm_response.data else 0
        current_approved = rm_response.data.get("total_approved_salons", 0) if rm_response.data else 0
        
        # Update RM profile
        db.table("rm_profiles").update({
            "total_score": current_score + score_change,
        ...
        # Add score history
        db.table("rm_score_history").insert({
            "rm_id": rm_id,
            "salon_id": salon_id,
            "score_change": score_change,
            "reason": f"Salon approved: {salon_name}"
        }).execute()
    ...
    async def reject_vendor_request(
        ...
        """
        # Get request
        request_response = db.table("vendor_join_requests").select("*").eq("id", request_id).single().execute()
        ...
        if rm_response.data and rm_response.data.get("profiles"):
            ...
            # Note: send_vendor_rejection_email is NOT async, so don't await it
            email_sent = email_service.send_vendor_rejection_email(
                to_email=rm_email,
                rm_name=rm_name,
                salon_name=request_data["business_name"],
                owner_name=request_data["owner_name"],
